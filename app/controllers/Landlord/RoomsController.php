	...
	public function check($room_sn)
	{
		try
		{
			...
			if($room->save())
			{
				...
				return Redirect::route('landlord.rooms', array($room->house->sn))->with('message', $message);
			}
			else
			{
				return Redirect::route('landlord.rooms', array($room->house->sn))->with('message', $room->errors());
			}
